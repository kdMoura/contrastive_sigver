...
def test_all_users(model: sklearn.svm.SVC,
    ...
    print("Skilled: ", all_metrics['EER'], all_metrics['EER_userthresholds'])
    print("Random: ", all_metrics['EER_rf'], all_metrics['EER_userthresholds_rf'])
    return results
...
def modelo_test(exp_set: Tuple[np.ndarray, np.ndarray, np.ndarray],
                         dev_set: Tuple[np.ndarray, np.ndarray, np.ndarray],
                         svm_type: str,
                         C: float,
                         gamma: float,
                         ...
                         fusion: str,
                         global_threshold: float = 0,
                         rng: np.random.RandomState = np.random.RandomState(),modelo=None) \
        -> Tuple[Dict[int, sklearn.svm.SVC], Dict]:
    """ Train and test classifiers for every user in the exploitation set,
        and returns the metrics.
    ...
    print('Training Writer-Independent (WI) classifier...')
    #_ = train_all_users(dev_train, svm_type, C, gamma, num_gen_train, rng)
    

    print('Tests have been performed:')
    results = test_all_users(modelo, exp_ref, exp_test, num_gen_test, fusion, global_threshold, rng)
