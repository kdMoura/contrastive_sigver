...
def split_train(dev_set: Tuple[np.ndarray, np.ndarray, np.ndarray],
    ...
    for user in users:
        #get all genuine signatures of writer = user
        user_genuines = np.flatnonzero((y == user) & (yforg == False))
        rng.shuffle(user_genuines)
        #randomly select genuine signature as many as num_gen_train
...
def create_training_set(dev_train: Tuple[np.ndarray, np.ndarray, np.ndarray],
    ...
    u_within_y = np.ones((u_within.shape[0])) #positive class

    G = S[:,:num_gen_train-1,:] #get all genuine, first num_gen_train signs
    u_between = pairwise_between_dissimilarity(G,num_gen_train//2,rng) #always agains half
    u_between_y = np.full((u_between.shape[0]),-1) #negative class
